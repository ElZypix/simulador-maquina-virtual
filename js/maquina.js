...
`;
    ...
    reiniciarMaquina() {
        ...
        this.Cx = 0;
        this.pila = [];
        localStorage.removeItem('mv_estado'); // Esto borra la memoria del navegador
        this.mostrarMensaje("Sistema: Memoria limpiada y registros en 0.");
        this.ActualizarPantalla();
    }
    ...
    exportarEstado() {
        // 1. Armar el contenido estructurado del archivo
        let contenido = "=== REPORTE DE ESTADO DE LA MÁQUINA VIRTUAL ===\n";
        contenido += `Fecha de exportación: ${new Date().toLocaleString()}\n\n`;
        
        contenido += "--- REGISTROS ---\n";
        contenido += `AX: ${this.Ax} (${this.formatearHex(this.Ax)}) [${this.formatearBin(this.Ax)}]\n`;
        contenido += `BX: ${this.Bx} (${this.formatearHex(this.Bx)}) [${this.formatearBin(this.Bx)}]\n`;
        contenido += `CX: ${this.Cx} (${this.formatearHex(this.Cx)}) [${this.formatearBin(this.Cx)}]\n\n`;

        contenido += "--- PILA ---\n";
        if (this.pila.length === 0) {
            contenido += "La pila está vacía.\n";
        } else {
            for (let i = this.pila.length - 1; i >= 0; i--) {
                contenido += `[Pos ${i}]: ${this.pila[i]}\n`;
            }
        }
        contenido += "\n===============================================\n";

        // 2. Crear el archivo virtual (Blob)
        ...
        this.mostrarMensaje("Sistema: Archivo .txt exportado correctamente.");
    }
    formatearHex(valor) {
        if (isNaN(valor) || valor === "ERR") return "0x0";
        return `0x${Math.floor(valor).toString(16).toUpperCase()}`;
    }
    ...
    exportarEstado() {
        ...
        contenido += `BX: ${this.Bx} (${this.formatearHex(this.Bx)}) [${this.formatearBin(this.Bx)}]\n`;
        contenido += `CX: ${this.Cx} (${this.formatearHex(this.Cx)}) [${this.formatearBin(this.Cx)}]\n\n`;
        contenido += `PILA: ${this.pila.length === 0 ? "Vacía" : this.pila.join(", ")}\n`;
        ...
        a.click();
        window.URL.revokeObjectURL(url);
        this.mostrarMensaje("Sistema: Reporte .txt generado.");
    }
}
